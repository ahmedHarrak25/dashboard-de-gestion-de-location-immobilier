...
const ModifyTenant = () => {
    ...
    return (
      <div className="add-tenant">
        <div className="add-tenant-header">
          ...
          <button className="back-btn" onClick={() => navigate("/tenants")}>
            ← Retour
          </button>
        </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:8080/api/locataires/${id}`, formData);
      navigate("/tenants");
    } catch (error) {
      console.error("Error updating tenant:", error);
      setError(
        "Erreur lors de la modification du locataire. Veuillez réessayer."
      );
    } finally {
      setSubmitting(false);
    }
